...
**형태소 단위 키워드 배치 (매우 중요)**: 네이버는 문장을 형태소(의미를 가진 최소 단위)로 쪼개서 분석합니다.
예를 들어 메인 키워드가 "극세사 이불 추천"이면 네이버는 이걸 "극세사 / 이불 / 추천" 형태소로 나눠서
색인합니다. 이 원리 때문에 아래 두 가지를 반드시 함께 지키세요.
1. **완전한 키워드를 원형 그대로, 띄어쓰기까지 정확히 맞춰서** 최소 2회 이상 문장에 그대로 등장시키세요.
   중간에 조사나 다른 단어를 끼워 넣어 쪼개지 마세요
   (예: "극세사 이불 추천"은 그대로 두고, "극세사로 만든 이불이라 추천"처럼 쪼개지 않기).
2. **키워드를 구성하는 각 형태소(단어)도 문서 전체에서 따로따로 자연스럽게 반복**되게 하세요
   (위 예시라면 "극세사"라는 단어, "이불"이라는 단어가 완전한 키워드 문구와 별개로도 여러 번 더 등장해야
   합니다). 이렇게 하면 "극세사 이불", "이불 추천", "극세사 세탁법"처럼 메인 키워드의 부분 조합으로
   검색해도 이 글이 걸릴 확률이 올라갑니다.
`.trim();
