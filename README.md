...
- **Phase 7 — 마무리**: 완료
  - 데모 이미지 캡션 중복 버그 수정(위 "Phase 7에서 고친 버그" 참고)
  - 모바일 390px 뷰포트에서 1~6단계 전체를 Playwright로 스크린샷 확인 — 가로 스크롤/
    뷰포트를 넘는 엘리먼트 없음을 실제로 검증
  - 4.5MB를 훨씬 넘는(16.8MB) 이미지를 업로드해 클라이언트 리사이즈가 실제로
    655KB까지 줄여 저장하는 것을 확인
  - 서버 API 라우트/프로바이더 어댑터/클라이언트 훅 전체에 `console.*` 호출이
    (에러 표시용 상태 변경 제외) 전혀 없음을 코드 검색으로 확인 — 키가 로그로
    새어나갈 경로가 구조적으로 없음
  - 1→6단계를 데모 모드로 한 번에 이어서 실행(스레드 포함, 더빙 ON)하고 ZIP까지
    내려받은 뒤 새로고침해도 완료 상태가 그대로 유지됨을 확인, 콘솔에 하이드레이션
    경고/페이지 에러 없음
  - 아래 "검수 체크리스트 결과"에 PROMPT.md 13절 항목별 실제 확인 결과를 정리
  - PROMPT.md 12절의 Phase 7 선택 기능인 [클립 업로드](직접 만든 비디오를 장면에 삽입)는
    구현하지 않았습니다 — 명시적으로 "선택"으로 표시된 기능이라 핵심 파이프라인
    완성을 우선했습니다.

## 배포 (Vercel)

1. 이 저장소를 Vercel에 새 프로젝트로 가져옵니다(Framework: Next.js 자동 감지).
2. 별도의 환경 변수 설정은 필요 없습니다 — 모든 API 키는 서버에 저장하지 않고
   사용자가 브라우저 설정 창에 직접 입력(BYOK)해서 요청 헤더로만 전달합니다.
3. 배포 후 다음을 한 번씩 직접 확인하세요(이 개발 환경은 egress 제한으로 못 했던 것들):
   - 폰트(Pretendard/Google Fonts)가 CDN에서 정상 로드되는지
   - MP4 변환 버튼이 unpkg CDN의 ffmpeg 코어를 정상적으로 불러오는지
   - 실제 API 키를 넣었을 때 텍스트/이미지/TTS가 정상 동작하는지

## API 키 발급

- Gemini: https://aistudio.google.com/apikey (무료 티어 제공)
- OpenAI: https://platform.openai.com/api-keys (유료, 크레딧 충전 필요)
- Hugging Face: https://huggingface.co/settings/tokens (무료 토큰, 월간 크레딧 한도 있음)
- Pexels: https://www.pexels.com/api/ (무료)

## 무료 한도 주의

- Gemini, Hugging Face, Pexels 모두 무료 티어에 호출량/크레딧 한도가 있습니다.
  한도를 넘기면 자동으로 QUOTA 오류로 분류되어 다음 프로바이더로 넘어가거나
  (이미지의 경우 Pollinations→Hugging Face 순으로), 텍스트/TTS는 유료 확인
  모달을 띄웁니다.
- Pollinations는 키 없이 계속 무료로 쓸 수 있지만 비공식/무보장 서비스라 언제든
  막힐 수 있습니다.
- Edge TTS(`msedge-tts`)는 비공식 API라 Microsoft 쪽에서 언제든 차단할 수
  있습니다 — 실패는 정상 시나리오로 설계했습니다(다음 프로바이더로 자동 전환).
- 이미지/영상 생성을 짧은 시간에 많이 시도하면(특히 Gemini) 429가 날 수 있어
  일괄 생성은 동시 2개로 제한해 뒀습니다.

## 검수 체크리스트 결과 (PROMPT.md 13절)

- [x] `npm run build`, `npx tsc --noEmit`, `npm run lint`, `npx vitest run`(48건) 모두 통과
- [x] 키 없이 데모 모드로 1→6단계 전체 완주(영상 다운로드 포함) — Playwright로 실제 확인
- [ ] Gemini 키만 있을 때 텍스트/이미지/더빙이 무료 체인으로 동작 — **실제 키로 확인 못 함**
      (이 환경엔 유효한 Gemini 키가 없고 egress도 제한적입니다). 체인 로직 자체는
      단위 테스트와 코드 리뷰로 검증했으니, 실제 키를 넣고 한 번 확인해 주세요.
- [x] OpenAI 키를 추가했을 때만 유료 폴백 모달이 뜨고, 취소하면 과금 호출이 발생하지
      않음 — 가짜 키로 재현해 모달 노출→취소 시 API 호출 없음을 확인
- [ ] 외부 URL 이미지를 넣어도 영상 녹화가 보안 오류 없이 성공(프록시 경유) —
      `/api/proxy-image`의 SSRF 방어와 캔버스 오염 방지 설계는 확인했지만, 이
      환경에서 실제로 접근 가능한 외부 이미지 URL이 없어 end-to-end로는 확인하지
      못했습니다.
- [x] 4.5MB 이상 이미지를 업로드해도 오류 없이 처리(클라이언트 리사이즈) — 16.8MB
      이미지 업로드 → 655KB로 리사이즈되어 저장됨을 확인
- [x] `localhost`, `127.0.0.1`, `169.254.169.254` 등을 `/api/proxy-image`에 넣으면
      거부됨 — 실행 중인 서버에 직접 curl로 확인
- [x] 새로고침 후에도 텍스트·이미지·영상 상태가 유지되고 하이드레이션 경고가 콘솔에
      없음 — 1→6단계 전체 완료 후 새로고침해서 확인
- [x] 콘솔/서버 로그 어디에도 API 키가 출력되지 않음 — API 라우트/프로바이더/클라이언트
      코드 전체에 `console.*` 호출이 없음을 확인(에러는 상태로만 표시)
- [x] 경험 기반 미선택 시 블로그 생성 불가 — 버튼이 비활성화됨을 확인
- [ ] 비경험 기반 글에 "직접 써봤다" 표현이 나오지 않음(샘플 3회 확인) — **실제 LLM
      호출로 확인 못 함**(유효한 API 키가 없음). 시스템 프롬프트(`RESEARCHED_GUIDE`)에
      해당 표현을 명시적으로 금지해 뒀지만, 실제 모델 출력 검증은 실제 키로 3회 이상
      생성해 직접 확인해 주세요.
- [x] 모바일 화면(폭 390px)에서 레이아웃이 깨지지 않음 — 1~6단계 전체를 390px
      뷰포트로 스크린샷 확인, 가로 스크롤 없음
- [x] 알려진 한계와 "확인 필요" 항목이 README에 정리됨 — 위 "확인 필요" 섹션
